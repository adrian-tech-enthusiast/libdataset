...
get_files_to_compile() {
  ...
  local header_files=""
  local source_files=""
  # Search headers and source files.
  for dir in $code_search_paths; do
    if [ -d "$dir" ]; then
      header_files="$header_files $(find $dir -maxdepth 3 -type f -name "*.h" ! -path '*/\.*' | sort)"
      source_files="$source_files $(find $dir -maxdepth 3 -type f -name "*.c" ! -path '*/\.*' | sort)"
    fi
  done
  # Return the list of files to compile using 'xargs' to clean up the output and remove any extra spaces.
  echo "$header_files $source_files" | xargs;
}
...
# Function to debug the project executable.
#
# This function runs the specified project executable through `gdb` (GNU Debugger),
# providing a stack trace to assist in debugging. It assumes the executable has been
# built and is located in the specified binary path. If the executable does not exist,
# the function suggests building the project first.
#
# Arguments:
#   $1: Base name for the project.
#       This is used to construct the name of the application executable (e.g., "project_base_name.app").
#   $2: Root path of the project.
#       This is used to locate the binary path where the executable resides.
#
# Usage:
#   debug_project "project_base_name" "/path/to/project";
debug_project() {
  # Validate arguments.
  if [ "$#" -ne 2 ]; then
    echo "Error: Invalid number of arguments.";
    echo "Usage: debug_project <base_name> <project_path>";
    exit 1;
  fi
  ...
  local base_name="$1"; # Base name for the project.
  local project_path="$2"; # Root path of the project.
  local bin_path="$project_path/bin"; # Path to the directory containing the executable.
  local app_name="$base_name.app"; # Name of the application executable.
  ...
  if [ ! -f "$executable" ]; then
    ...
    echo "Please build the project first by running the script '.github/build.sh'.";
    exit 1;
  fi

  # Run the executable through gdb, print the stack trace and then quit.
  # gdb flags used:
  # 1. --quiet: Suppresses unnecessary output from gdb.
  # 2. -ex 'set confirm off': Disables confirmation prompts in gdb.
  # 3. -ex run: Runs the executable.
  # 4. -ex backtrace: Prints a stack trace of the current execution.
  # 5. -ex quit: Quits gdb after printing the stack trace.
  gdb --quiet -ex 'set confirm off' -ex 'run' -ex 'backtrace' -ex 'quit' "$executable";
}
